...
saved_model=load_model(r'D:\Loss_Visual\ResNet20_NOSHCUT_WD_B128\saved_models\cifar10_ResNet20_NOSHCUT_model_158.h5')
...
def calulate_loss_landscape(model, directions):
    setup_surface_file()
    with h5py.File("./3d_surface_file_ResNet20_NOSHCUT.h5", 'r+') as f:
...
def setup_surface_file():
    ...
    surface_path = "./3d_surface_file_ResNet20_NOSHCUT.h5"

    if os.path.isfile(surface_path):
        print("%s is already set up" % "3d_surface_file_ResNet20_NOSHCUT.h5")

        return

    with h5py.File(surface_path, 'a') as f:
        print("create new 3d_sureface_file_ResNet20_NOSHCUT.h5")
